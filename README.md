...
1. **平台**：定位、核心数字、能力入口、15 步闭环链路、证据状态
2. **工作台**：漂移诊断（上传时序 → 冻结核心实时判别 → JSON 报告）+ 决策安全盾五门交互演示
3. **证据**：T287 漂移感知与判别（78 快照、E0/E1、更新周期经济学）+ T176 闭环终测（三证据层、配对残差、置换直方图、**在线复算**）
4. **方法**：哈密顿量、AEMTN 架构、训练摘要、AI 框架选型 + 数据集 / 特征契约、三 seed 权重
5. **边界与复现**：能说 / 须限定 / 不可说三档、技术瓶颈、未部署项 + 本地复现与完整性校验步骤
